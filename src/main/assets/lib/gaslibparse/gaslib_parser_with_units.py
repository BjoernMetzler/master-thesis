...
class GasLibParserUnits(object):
    ...
    def _parse_net_file(self):
        ...
        connections_element = etree.find(
            "framework:connections", self.namespaces)
    ...
    def _parse_innodes(self, innodes_iter):
        for innode in innodes_iter:
            ...
            node_dict["pos"] = [float(innode.get('x')), float(innode.get('y'))]
            for element in innode.iter():
                ...
                node_dict[new_name] = self._parse_value(
    ...
    def _parse_sinks(self, sinks_iter):
        for sink in sinks_iter:
            ...
            exit_dict["pos"] = [float(sink.get("x")), float(sink.get("y"))]
            for element in sink.iter():
                ...
                if tag == "sink":
                    ...
                    new_name = self.cc_to_us(tag)
                exit_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
        ...
        for source in sources_iter:
            ...
            for element in source.iter():
                ...
                if tag == "coefficient-A-heatCapacity":
                    ...
                    new_name = self.cc_to_us(tag)
                entry_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_connections(self, connections_element):
        ...
        pipes_iter = connections_element.iterfind(
            "gas:pipe", self.namespaces)
        self._parse_pipes(pipes_iter)
        compressors_iter = connections_element.iterfind(
            "gas:compressorStation", self.namespaces)
        self._parse_compressor_stations(compressors_iter)
        cvalves_iter = connections_element.iterfind(
            "gas:controlValve", self.namespaces)
        self._parse_control_valves(cvalves_iter)
        valves_iter = connections_element.iterfind(
            "gas:valve", self.namespaces)
        self._parse_valves(valves_iter)
        spipes_iter = connections_element.iterfind(
            "gas:shortPipe", self.namespaces)
        self._parse_short_pipes(spipes_iter)
        resistors_iter = connections_element.iterfind(
            "gas:resistor", self.namespaces)
        ...
        for compressor in compressors_iter:
            ...
            for element in compressor.iter():
                ...
                if tag == "compressorStation":
                    ...
                    new_name = self.cc_to_us(tag)
                compressor_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_control_valves(self, cvalves_iter):
        for cvalve in cvalves_iter:
            cvalve_dict = {}
            cvalve_dict["arc_id"] = cvalve.get("id")
            cvalve_dict["from_node"] = cvalve.get("from")
            cvalve_dict["to_node"] = cvalve.get("to")
            for element in cvalve.iter():
                ...
                if tag == "controlValve":
                    ...
                    new_name = self.cc_to_us(tag)
                cvalve_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_valves(self, valves_iter):
        for valve in valves_iter:
            valve_dict = {}
            valve_dict["arc_id"] = valve.get("id")
            valve_dict["from_node"] = valve.get("from")
            valve_dict["to_node"] = valve.get("to")
            for element in valve.iter():
                ...
                if tag == "valve":
                    ...
                    new_name = self.cc_to_us(tag)
                valve_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
        ...
        for pipe in pipes_iter:
            ...
            for element in pipe.iter():
                ...
                if tag == "pipe":
                    ...
                    new_name = self.cc_to_us(tag)
                pipe_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_short_pipes(self, spipes_iter):
        for spipe in spipes_iter:
            spipe_dict = {}
            spipe_dict["arc_id"] = spipe.get("id")
            spipe_dict["from_node"] = spipe.get("from")
            spipe_dict["to_node"] = spipe.get("to")
            for element in spipe.iter():
                ...
                if tag == "shortPipe":
                    ...
                    new_name = self.cc_to_us(tag)
                spipe_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_resistors(self, resistors_iter):
        for resistor in resistors_iter:
            resistor_dict = {}
            resistor_dict["arc_id"] = resistor.get("id")
            resistor_dict["from_node"] = resistor.get("from")
            resistor_dict["to_node"] = resistor.get("to")
            for element in resistor.iter():
                ...
                if tag == "resistor":
                    ...
                    new_name = self.cc_to_us(tag)
                resistor_dict[new_name] = self._parse_value(
                    element.get("value"), element.get("unit"))
    ...
    def _parse_scn_node(self, node_element):
        scn_intervals = {"pressure":
                         (unit.Quantity(0, "bar"),
                          unit.Quantity(np.Infinity, "bar")),
                         "flow":
                         (unit.Quantity(0, "m_cube_per_hour"),
                          unit.Quantity(np.Infinity, "m_cube_per_hour"))}
        ...
        for bound in node_element.iter():
            ...
            bound_value = self._parse_value(
                bound.get("value"), bound.get("unit"))
            # print(bound_value)
        ...
        net_pressure_interval = (
            node_data['pressure_min'],
            node_data['pressure_max']
        )
        result_pressure_interval = self._intersect_intervals(
            scn_intervals["pressure"], net_pressure_interval)
    ...
    def _parse_value(self, value_string, unit_string):
    ...
    #     # using a generic compressor dict for now #TODO check this later!
    #     else:
    #         compressor_dict = {"max_pressure_increase": 31.70737918,
    #                            "max_pressure_ratio": 1.806683035,
    #                            "min_pressure_increase": 1.034326951,

...
# Third-party
from lib.gaslibparse.gaslib_parser_with_units import GasLibParserUnits
...
class GasLibConverter(object):
    ...
    def __init__(self, net_file, scn_file, gams_data_file):
        ...
        self.gaslib_parser = GasLibParserUnits(self.net_file, self.scn_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='GasLib to GAMS data Converter.')
    parser.add_argument("gaslib_net_filename",
                        help="GasLib network description filename.")
    parser.add_argument("gaslib_scn_filename",
                        help="GasLib scenario description filename.")
    ...
    with open(args.gaslib_net_filename, 'r') as gaslib_net_file:
        with open(args.gaslib_scn_filename, 'r') as gaslib_scn_file:
            with open(args.gams_data_filename, 'w') as gams_data_file:
                gaslib_converter = GasLibConverter(
                    gaslib_net_file, gaslib_scn_file, gams_data_file)
